  ...
  try {
    ...
    res.json({
      ...
      keyPreview: hasApiKey
        ? `${user.cohereApiKey.slice(0, 4)}...${user.cohereApiKey.slice(-4)}`
        : null,
    });
  } catch (err) { next(err); }
};
...
// ─── GET /users/plan-status ───────────────────────────────────────────────────
// Frontend uses this to show quota bar, lock UI, and conditionally show upgrade CTA
export const getPlanStatus = async (req: AuthRequest, res: Response, next: NextFunction) => {
  try {
    const user = await User.findById(req.user!.id);
    if (!user) throw new ApiError("User not found.", 404);
    ...
    res.json({
      ...
      proposalsRemaining: isPro ? null : FREE_MONTHLY_CAP - user.proposalsThisMonth,
      monthlyLimit:       isPro ? null : FREE_MONTHLY_CAP,
      features: {
        unlimitedProposals: isPro,
        advancedScoring:    isPro,
        pdfExport:          isPro,
